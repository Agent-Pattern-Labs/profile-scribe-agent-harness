...
const tmp = mkdtempSync(join(tmpdir(), 'profilescribe-rig-topic-guard-'));
const createPostCalls = [];

const sources = [
  {
    id: 'src-projects-guard',
    kind: 'website',
...
const server = createServer(async (request, response) => {
  ...
  if (name === 'read_profile') {
    ...
    result = { draft: { id: 'draft-topic-guard-smoke' } };
  } else {
...
try {
  ...
  writeFileSync(jobFile, `${JSON.stringify({
    id: 'job-topic-guard-smoke',
    kind: 'draft_post',
    payload: {
      topic: 'publish one more profile scribe post',
      maxSources: 3
    }
  })}\n`, 'utf8');
...
process.stdin.on('end', () => {
  ...
  process.stdout.write(JSON.stringify({
    topic: 'publish one more profile scribe post',
    body: 'Quick work-in-progress note: the public pages around current projects still point to the same builder pattern.',
    abstracts: ['Current projects still share a builder pattern'],
    tone: 'professional',
    sourceIds: ['src-projects-guard']
  }));
});
`, 'utf8');
  ...
  if (createPostCall.topic === 'publish one more profile scribe post') {
    throw new Error('post instruction leaked into submitted timeline topic');
  }
  if (createPostCall.topic !== 'Current projects still share a builder pattern') {
    throw new Error(`expected abstract-derived topic, got ${JSON.stringify(createPostCall.topic)}`);
  }

  console.log('profile-scribe-rig topic guard smoke check passed.');
} finally {
  server.close();
